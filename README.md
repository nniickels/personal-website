...
| Playground animation rate | Uses the display’s normal refresh cadence. Offscreen experiments are still paused by `IntersectionObserver`. | JavaScript experiment updates and continuous CSS visual steps are capped at approximately 30 FPS to reduce mobile CPU and GPU load. |
| Black Hole experiment | Includes the Variables Guide, all variable sliders, growth playback, the draggable mass-growth plot, presets, results, and 3D rotation. | Uses a lighter render path with presets, results, 3D rotation, and a concise preset-aware explanation. Variable sliders, growth playback, the Variables Guide, and the plot are not mounted. |
